...
public class ListView extends Div {
    ...
    private Grid.Column<Client> activeColumn;
    ...
    private void createActiveColumn() {
        activeColumn = grid.addEditColumn(Client::isActive, new ComponentRenderer<>(client -> {
                    Span span = new Span();
                    span.setText("" + client.isActive());
                    return span;
                })).checkbox((item, newValue) -> item.setActive(newValue))
                .setComparator(client -> client.isActive()).setHeader("Active");
    }
    ...
    private List<Client> getClients() {
        return Arrays.asList(
                ...
                createClient(1077, "https://randomuser.me/api/portraits/men/94.jpg", "Stephanus Huggins", 75774.0,
                        "Success", "2019-02-26", true),
                createClient(8942, "https://randomuser.me/api/portraits/men/16.jpg", "Torsten Paulsson", 82531.0,
                        "Pending", "2019-02-21", false));
    }

    private Client createClient(int id, String img, String client, double amount, String status, String date, boolean active) {
        Client c = new Client();
        c.setId(id);
        c.setImg(img);
        c.setClient(client);
